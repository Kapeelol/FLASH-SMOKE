# FLASH KRATOM — สั่งพอตในตัวเมืองชุมพร (Full System v5)
...
| คนส่งของ | `0811111111` | `rider1234` |

## ฟีเจอร์ใหม่ v9

- **บัญชีคนส่งของ (rider)** — ล็อกอินแล้วเห็นหน้าเหมือนแอดมินแต่**เฉพาะหน้าออเดอร์** (ไม่เห็นแท็บสินค้า/ยอดขาย/ตั้งค่า) อัปเดตสถานะ + กดนำทาง Google Maps ได้
- **แยกประเภทออเดอร์** — หน้าออเดอร์มีแท็บกรอง: ทั้งหมด / 🟢 ลูกค้าไลน์ / 📱 สมาชิก / 🚫 ยกเลิกแล้ว (แต่ละออเดอร์มีป้ายบอกว่ามาจาก LINE หรือสมัครสมาชิก)
- **ล็อกออเดอร์ที่ยกเลิก** — ออเดอร์ที่ยกเลิกแล้วจะย้ายไปแท็บ "ยกเลิกแล้ว" และแก้ไขสถานะไม่ได้ (บล็อกทั้งหน้าเว็บและเซิร์ฟเวอร์)
- **แก้หน้าจอเด้งขึ้นบนสุด** — ตอนกดเพิ่มสินค้าลงตะกร้า (หน้าเมนู) และตอนเลือกในหน้าชำระเงิน หน้าจอจะไม่เด้งกลับขึ้นบนสุดอีก (จำตำแหน่ง scroll ไว้)

> ⚠️ ถ้าใช้ Supabase อยู่แล้ว ต้องรัน migration เพิ่มคอลัมน์: `alter table orders add column if not exists customer_source text default 'phone';`

## ฟีเจอร์ v8

- **แบนเนอร์หน้าแรกแบบสไลด์** — กรอบใหญ่หน้าแรกเปลี่ยนเป็นสไลด์รูปภาพ (ปัดซ้าย-ขวา + เลื่อนอัตโนมัติ + จุดบอกตำแหน่ง) เอาข้อความ "พร้อมส่งแล้ววันนี้ / สั่งพอตในตัวเมืองชุมพร / ส่งฟรีเมื่อครบ 2 ตัว" ออกแล้ว เหลือแต่รูป
- **แอดมินจัดการแบนเนอร์** — เมนูแอดมิน (แท็บสินค้า) เพิ่มการ์ด "แบนเนอร์หน้าแรก": อัปโหลดรูปได้หลายรูป / ลบได้ทีละรูป → เปลี่ยนสไลด์หน้าแรกทันที (ผ่าน SSE)

## ฟีเจอร์ v7

- **บ้านเลขที่ + ตำแหน่งแม่นขึ้น** — หน้าปักหมุดเพิ่มช่อง "บ้านเลขที่ / ชื่ออาคาร" (บังคับกรอก) นำไปต่อหน้าที่อยู่จากแผนที่ให้ครบ; เปิดแผนที่ครั้งแรกจะขอ GPS อัตโนมัติเพื่อไม่ให้หมุดเริ่มกลางเมือง (มั่ว); ตอนกดบันทึกจะค้นที่อยู่ของพิกัดล่าสุดอีกครั้งเพื่อกันข้อความค้างจากตำแหน่งเก่า
- **ยืนยันเบอร์ก่อนสั่งซื้อ** — ลูกค้าที่ล็อกอินด้วย LINE (ยังไม่มีเบอร์) จะมี pop up ขึ้นให้ใส่เบอร์ตั้งแต่เข้าเว็บครั้งแรก; ถ้ายังไม่มีเบอร์จะกดสั่งซื้อ/ชำระเงินไม่ได้ (บล็อกทั้งฝั่งหน้าเว็บและเซิร์ฟเวอร์)
...
- **โลโก้จริง** — ใช้รูปโลโก้ FLASH KRATOM (`public/assets/logo.png`) ที่หน้า welcome และมุมซ้ายบนหน้าแรก (เอาตัวอักษร FLASH KRATOM เดิมออก)
...
4. ตั้ง env บน Render:
   ...
   | `MAIL_FROM_NAME` | `FLASH KRATOM` (หรือชื่อร้าน) |
